 ...
 * 
 * This is a custom variant of MPU6050_light library
 * by Pavel Kochetkov (github.com/Wosk1947)
 */
...
byte MPU6050::begin(int gyro_config_num, int acc_config_num, int angConf){
...
void MPU6050::fetchData(){
  ...
  rawAccGyro.ax = rawData[0];
  rawAccGyro.ay = rawData[1];
  rawAccGyro.az = rawData[2];
  rawAccGyro.gx = rawData[4];
  ...
  if (angleRemap == 2) {
	rawData[0] = rawAccGyro.ax;
	rawData[1] = rawAccGyro.az;
	rawData[2] = -rawAccGyro.ay;
	rawData[4] = rawAccGyro.gx;
	rawData[5] = rawAccGyro.gz;
	rawData[6] = -rawAccGyro.gy;
  }
...
void MPU6050::update(){
  ...
  //normalize ax,az
  float m = sqrt(accZ * accZ + accX * accX);
  float accXm = accX;
  float accZm = accZ;
  if (m != 0 && !isnan(m)) {
	  accXm /= m;
  ...
  angleAccX =   atan2(accY, /*sgZ*/sqrt(accZm * accZm + accXm * accXm)) * RAD_2_DEG; // [-180,+180] deg
  angleAccY = - atan2(accX,     sqrt(accZm*accZm + accY * accY )) * RAD_2_DEG; // [- 90,+ 90] deg
